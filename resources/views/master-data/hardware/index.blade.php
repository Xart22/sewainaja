...
<script>
    ...
    return {
        ...
        isOpen() { return this.show === true },
        selectedName() { return this.selected ? this.selected.name + ' ' + this.selected.group_name : this.filter; },
        classOption(id, index) {
          const isSelected = this.selected ? (id == this.selected.id) : false;
          const isFocused = (index == this.focusedOptionIndex);
          return {
          ...
          return this.options
            ? this.options.filter(option => {
                return (option.name.toLowerCase().indexOf(this.filter) > -1) 
                  || (option.group_name.toLowerCase().indexOf(this.filter) > -1)
        ...
        onOptionClick(index) {
          this.focusedOptionIndex = index;
          document.getElementById('customer_id').value = this.filteredOptions()[index].id;
          this.selectOption();
        },
        selectOption() {
          if (!this.isOpen()) {
